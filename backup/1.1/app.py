...
def seed_db_command():
    ...
    try:
        ...
        print("\nSeed data berhasil.")
        print("Hanya akun SuperOwner yang dibuat.")
        print("-------------------------------------------------")
        print(f"Username : {super_owner.username}")
        print(f"Password : {super_owner.password}")
        print("-------------------------------------------------")
...
def get_supplier_history(supplier_id):
    try:
        ...
        # --- PERBAIKAN ISOLASI: Filter Lapak Berdasarkan Owner dari Supplier ---
        ...
        # Hanya ambil lapak milik Owner yang sama dengan Supplier ini
...
# --- TAMBAHKAN INI DI BAGIAN SUPPLIER API (Sekitar baris 1390-an) ---

@app.route('/api/get_supplier_unpaid_details/<int:supplier_id>', methods=['GET'])
def get_supplier_unpaid_details(supplier_id):
    try:
        # Kita ambil laporan yang statusnya 'Terkonfirmasi'.
        # Dalam sistem ini, 'Terkonfirmasi' berarti Owner sudah menyetujui laporan
        # dan nominalnya masuk ke saldo hutang (balance) supplier.
        
        # Opsi: Anda bisa membatasi hanya bulan ini jika data terlalu banyak
        # today = datetime.date.today()
        ...
        query = db.session.query(
         ...
         .filter(
             Product.supplier_id == supplier_id,
             LaporanHarian.status == 'Terkonfirmasi'
             # , LaporanHarian.tanggal >= start_date # Uncomment jika ingin filter per bulan
         )\
         .group_by(LaporanHarian.tanggal, Lapak.lokasi, Product.nama_produk)\
         .order_by(LaporanHarian.tanggal.desc())

        results = query.all()

        details = []
        # Mengelompokkan hasil agar jika ada beberapa produk di lapak/tanggal sama, terlihat rapi
        # Namun untuk simplifikasi tabel, kita kirim baris per produk atau per lapak
