...
from app.models import Admin, SuperOwner, SuperOwnerBalance, LaporanHarian, Lapak, RiwayatPenarikanSuperOwner

# Buat Blueprint
superowner_bp = Blueprint('superowner', __name__)

@superowner_bp.route('/api/get_superowner_dashboard_data/<int:superowner_id>', methods=['GET'])
def get_superowner_dashboard_data(superowner_id):
    try:
        ...
        db.session.rollback()
        print(f"Error getting SO dashboard data: {str(e)}")
        return jsonify({"success": False, "message": f"Gagal mengambil data dashboard: {str(e)}"}), 500

@superowner_bp.route('/api/get_superowner_owners/<int:superowner_id>', methods=['GET'])
...
@superowner_bp.route('/api/get_superowner_profit_details/<int:owner_id>', methods=['GET'])
...
@superowner_bp.route('/api/get_superowner_report_profit_detail/<int:report_id>')
def get_superowner_report_profit_detail(report_id):
    try:
        report = LaporanHarian.query.options(joinedload(LaporanHarian.lapak)).get(report_id)
        if not report:
...
@superowner_bp.route('/api/superowner_withdraw', methods=['POST'])
...
@superowner_bp.route('/api/superowner_withdraw_from_owner', methods=['POST'])
def superowner_withdraw_from_owner():
    ...
    try:
        ...
        return jsonify({"success": True, "message": f"Saldo berhasil ditandai sebagai lunas."})
    except Exception as e:
        db.session.rollback()
        return jsonify({"success": False, "message": "Gagal memproses penarikan."}), 500

@superowner_bp.route('/api/get_superowner_profit_reports/<int:superowner_id>', methods=['GET'])
...
@superowner_bp.route('/api/get_superowner_transactions/<int:superowner_id>', methods=['GET'])
def get_superowner_transactions(superowner_id):
    try:
        ...
        if start_date: profit_query = profit_query.filter(LaporanHarian.tanggal >= start_date)
        if end_date: profit_query = profit_query.filter(LaporanHarian.tanggal <= end_date)
        ...
        # 2. Penarikan Keluar
        payout_query = RiwayatPenarikanSuperOwner.query.options(joinedload(RiwayatPenarikanSuperOwner.owner)).filter(RiwayatPenarikanSuperOwner.super_owner_id == superowner_id)
        if start_date: payout_query = payout_query.filter(func.date(RiwayatPenarikanSuperOwner.tanggal_penarikan) >= start_date)
        if end_date: payout_query = payout_query.filter(func.date(RiwayatPenarikanSuperOwner.tanggal_penarikan) <= end_date)
        ...
        tx_list = [{**item, "tanggal": item['tanggal'].strftime('%Y-%m-%d')} for item in all_transactions]
...
@superowner_bp.route('/api/get_superowner_owner_reports/<int:superowner_id>', methods=['GET'])
def get_superowner_owner_reports(superowner_id):
    try:
        start_date_str = request.args.get('start_date')
        end_date_str = request.args.get('end_date')
        start_date, end_date = None, None
        if start_date_str: start_date = datetime.datetime.strptime(start_date_str, '%Y-%m-%d').date()
        if end_date_str: end_date = datetime.datetime.strptime(end_date_str, '%Y-%m-%d').date()
        ...
        for owner in owners:
            ...
            if not lapak_ids:
                owner_reports.append({"owner_id": owner.id, "owner_name": owner.nama_lengkap, "lapak_names": [], "total_biaya_supplier": 0, "total_keuntungan_owner": 0, "total_keuntungan_superowner": 0})
                continue
            ...
            ).filter(LaporanHarian.lapak_id.in_(lapak_ids), LaporanHarian.status.in_(['Terkonfirmasi', 'Difinalisasi']))
            
            if start_date: query = query.filter(LaporanHarian.tanggal >= start_date)
            if end_date: query = query.filter(LaporanHarian.tanggal <= end_date)
        ...
        return jsonify({"success": False, "message": "Gagal mengambil laporan agregat."}), 500