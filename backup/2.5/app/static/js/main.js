...
async function handleKirimLaporan() {
  // --- PERBAIKAN: Validasi Ulang Sebelum Kirim ---
  
  // 1. Hitung ulang Total Sistem
  let totalSistem = 0;
  document.querySelectorAll(".product-row").forEach((row) => {
    const stokAwal = parseInt(row.querySelector(".stok-awal").value) || 0;
    const stokAkhir = parseInt(row.querySelector(".stok-akhir").value) || 0;
    ...
    const terjual = Math.max(0, stokAwal - stokAkhir);
    totalSistem += terjual * hargaJual;
  });

  // 2. Hitung ulang Total Manual
  const qris = parseFloat(document.getElementById("rekap-qris").value.replace(/\D/g, '')) || 0;
  const bca = parseFloat(document.getElementById("rekap-bca").value.replace(/\D/g, '')) || 0;
  const cash = parseFloat(document.getElementById("rekap-cash").value.replace(/\D/g, '')) || 0;
  const totalManual = qris + bca + cash;

  // 3. Cek Selisih (Tanpa Toleransi)
  const isMatched = totalSistem === totalManual;

  if (totalSistem === 0) {
     return showToast("Belum ada produk yang terjual atau stok belum diisi.", false);
  }

  if (!isMatched) {
    // Tampilkan pesan error dan batalkan pengiriman
    const selisih = totalManual - totalSistem;
    return showToast(`Nominal belum sesuai! Selisih: ${formatCurrency(selisih)}`, false);
  }
  // ------------------------------------------------
  ...
  const rekapData = {
    qris: qris, // Gunakan variabel yang sudah di-clean di atas
    bca: bca,
    cash: cash,
    total: totalManual,
  };
